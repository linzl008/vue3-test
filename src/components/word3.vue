...
function startDrawTextAndHeart (container, heartCan, y, x, texts) {
  return new Promise(resolve => {
    const textAnimInfos = texts.split('').map((item, index) => {
      const text = new createjs.Text(item, calPx(50) * getRatio() + 'px myfont', '#4B4B6A')
      text.x = x
...
export default {
  ...
  mounted () {
    ...
    //     await startDrawTextAndHeart(container, heartCan, 40 * getRatio(), calPx(450 - 60 * i) * getRatio(), heartCtx1)
    //   }
    // })()
    //
    // function tick () {
